...
## PI(3,4,5)P3 pocket conservation check (ATP8B1 → ATP8B4)

Per Dieudonné et al. 2023 (Nat Commun 14:7492), the PI(3,4,5)P3 regulatory
site of ATP8B1 is formed by TM5, TM7, TM8, TM10, with primary contacts
R952, R1032, R1164 (ATP8B1 numbering). This site is distinct from the
transport-pathway lipid site (TM1-4) occupied by PIE in the 8OXC template.

### Files
- `13_check_pip3_pocket_conservation.py` — maps ATP8B1 R952/R1032/R1164 to
  the aligned ATP8B4 position via `alignment.ali` and reports the residue
  identity at each position

### Results
- R952 (ATP8B1) → Cys (ATP8B4) — not conserved
- R1032 (ATP8B1) → not an exact match, but Arg/Lys present adjacent in sequence
- R1164 (ATP8B1) → conserved (Arg)
