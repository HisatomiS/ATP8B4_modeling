...
target_resnum = dktgt[3][0]  # the G in D-K-T-G-T
print(f"\nMutation target: Gly{target_resnum} -> Ser")
...
# --- Spatial neighbor search on WT structure (unbiased) ---
...
# --- Distance check: WT CA vs mutant CB/OG, for acidic (Asp/Glu) neighbors ---
...
for chain_id, resnum in acidic_neighbors:
    ...
    for atom_name in ("CB", "OG"):
        ...
        print(f"  MUT {atom_name}(Ser{target_resnum}) - {mut_other.get_resname()}{resnum}  min distance = {mut_dist:.2f} A{flag}")
